 ...
 *
 * >>> EDIT THE CELL MAP BELOW TO MATCH YOUR TEMPLATE <<<
 * Nothing else in this file should need to change when you update
 * your template layout.
 *
 * EPF (11% employee / 13% employer) is always recalculated here on
 * the server from Net Salary — never trusted from the browser — so
 * the numbers can't be tampered with client-side. When the EPF/SOCSO
 * toggle is off, every EPF/SOCSO/EIS cell (both employee and
 * employer side) is forced to 0.
 * ---------------------------------------------------------------
 */
...
// Human-readable month names, used because the form only submits month as 1-12
const MONTH_NAMES = [
...
// --- Payment Date, e.g. "1 August 2026" (input type=date sends YYYY-MM-DD) ---
...
// --- EPF / SOCSO / EIS, server-side authoritative calculation ---
if ($epfSocsoEnabled) {
    ...
    // Staff opted out of EPF/SOCSO — everything on both sides is zero.
...
// Fixed core insertion block with label checking logic
foreach (CELL_MAP as $field => $cellRef) {
    $valueToWrite = $values[$field] ?? '';
    
    // Check if the current field has an assigned label prefix
    if ($valueToWrite !== '' && isset(CELL_LABELS[$field])) {
        // Appends a single space after your configured text label
